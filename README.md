...
It is a list of actions, and each action is a dictionary that contains the source node, the destination node, and a list edges needed to be traversed to go from the source to destination.
...
The goal is to minimize the amount of blocking. Therefore, a reward of `+1` is given for successfully finding a path that has an available color. Conversely, if an action is taken and no available color is found, then the reward is `-1`. Finally, if an action is taken and it does not match the incoming request (EX: incoming request is src=0 dest=4, but the action taken is for src=7 dest=3) then the reward is `-1`.
...
Initially, the averaged episode rewards started at ~58000 and showed gradual growth. It peaked by episode 4 at ~71000 averaged rewards. After this point, the averaged rewards starts to slowly decline. The reason for this curve is because the agent quickly learns which actions to take early on, and then it rapidly starts to gain rewards. Then based on these rewards, the agent knows what actions to take and adapts slower to varying situations. As it begins to stick with what it thinks is the best action rather than looking for a better action, the average rewards declines when encountering episodes with different needs.  This demonstrates the need for data curation/variety to combat "overfitting".
...
This plot correctly shows the inverse of the previous plot. As the agent is exploring for the best actions early on, the initial averaged objective *B* is very high at 0.21. Gradually the agent learns better actions to take and the average objective *B* declines to a minimum of ~0.145 by episode 4. From then on, the averaged objective *B* increases again as the agent is less likely to look for better actions.
...
This plot shows that, on average, objective *B* is between 0.0 and 0.010. There are intermittent spikes where objective *B* rises to 0.015 or even 0.025, but these are rare. These results show that the agent can consistently assign paths in a manner that minimizes objective *B* when the capacity = 20.
...
The learning curve initially starts at ~5000 averaged rewards and slowly grows. It begins to plateau at episode 3 where the averaged episode rewards is still increasing, but at a slower rate. Something noticeable is that the maximum episode rewards here is ~45000 whereas for capactity=20 it was ~71000. This is natural as there is less exploration to be done with less capacity, and thus less rewards to be gained. The agent quickly learns the optimal routing strategy and then the averaged rewards plateaus.
...
The averaged objective *B* is initially at a very high ~0.48. This is expected as it is more difficult to efficiently route when the capacity is 10. As the agent learned, the averaged objective *B* quickly reached ~0.25. We can infer that the averaged objective *B* begins to stablize at ~0.25 because the agent learns an optimal routing policy for capacity 10 and it is not able to improve as quickly.
